...
async function sliceAndGoToExport(
  ...
  await expect(page.getByRole('button', { name: 'Episode Export' }).first()).toBeVisible({ timeout: 20_000 })

  await page.getByRole('button', { name: 'Episode Export' }).first().click()
...
test('export page shows country and platform sections', async ({ electronApp, page, testBaseDir }) => {
  ...
  await expect(page.getByRole('columnheader', { name: 'Width' })).toBeVisible()
...
test('exports to multiple platforms and shows results', async ({ electronApp, page, testBaseDir }) => {
  ...
  const runExportBtn = page.getByRole('button', { name: 'Run Export' })
  await expect(runExportBtn).toBeEnabled({ timeout: 5_000 })
  await runExportBtn.click()

  // 내보내기 완료 대기
  await expect(page.getByRole('button', { name: 'Open Export Folder' }).first()).toBeVisible({ timeout: 30_000 })
...
test('re-export shows already exported badges', async ({ electronApp, page, testBaseDir }) => {
  ...
  await expect(page.getByText('Exported').first()).toBeVisible({ timeout: 5_000 })
})
