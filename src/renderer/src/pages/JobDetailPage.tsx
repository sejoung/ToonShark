...
export default function JobDetailPage() {
  ...
  if (!currentJob || currentJob.id !== jobId) {
    return <div className="p-6 text-slate-400">{t.jobNotFound}</div>
  }
      ...
      <div className="bg-slate-800 rounded-lg p-4 mb-6 border border-slate-700">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <p className="text-white">{new Date(currentJob.createdAt).toLocaleString()}</p>
          </div>
          <div>
            <span className="text-slate-400">{t.pages}</span>
            <p className="text-white">{currentJob.pageCount}</p>
          </div>
          <div>
            <span className="text-slate-400">{t.slicesLabel}</span>
            <p className="text-white">{currentJob.sliceCount}</p>
          </div>
          <div>
            <span className="text-slate-400">{t.mode}</span>
            <p className="text-white">{currentJob.mode === 'fixed' ? t.fixedInterval : t.auto}</p>
          </div>
          <div>
            <span className="text-slate-400">{t.prefixLabel}</span>
            <p className="text-white">{currentJob.prefix}</p>
          </div>
          <div className="col-span-2">
            <span className="text-slate-400">{t.sourcePdfLabel}</span>
            <p className="text-white truncate">{currentJob.sourcePdfPath.split(/[\\/]/).pop()}</p>
          </div>
        </div>
      </div>
      ...
      <div className="columns-4 md:columns-6 lg:columns-8 gap-3">
        {currentJob.files.map((file) => (
          <button
            ...
            className="mb-3 break-inside-avoid bg-slate-800 rounded-lg overflow-hidden border border-slate-700 cursor-pointer hover:border-blue-500 transition text-left w-full"
          >
            <LazyImage
